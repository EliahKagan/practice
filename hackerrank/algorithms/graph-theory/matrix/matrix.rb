...
if __FILE__ == $PROGRAM_NAME
  ...
  roads = Array.new(city_count - 1) { Edge.new(*read_record) }

  # Read if each vertex (city) has is hot (has a machine) or cold (no machine).
  heats = [false] * city_count
  machine_count.times { heats[gets.to_i] = true }
  sets = HeatTrackingDisjointSets.new(heats)

  # Taking edges by decreasing weight, sum weights that connect hot components.
