...
# operation.
#
# This differs from median-updates.cr by avoiding the need to capture blocks.
# I had expected this to be faster, by inlining the comparisons, but that
# doesn't appear to be the case.

# Represents natural comparisons for any comparable type.
...
# Represents reversed comparisons for any comparable type.
...
# heap. Calling code is responsible for determining that the comparer, C, is
# compatible with equality (== and hashing) on the element type T.
class BinaryHeap(T, C)
  def initialize(@comp = C.new)
  end
  ...
  private def order_ok?(parent_value, child_value)
    @comp.compare(parent_value, child_value) <= 0
  end
...
class MedianBag
  ...
  @low = BinaryHeap(Int32, ReverseComparer).new  # Max-heap of lesser values.
  @high = BinaryHeap(Int32, NaturalComparer).new # Min-heap of greater values.
end
