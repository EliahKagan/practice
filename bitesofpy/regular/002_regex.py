...
HASHTAG_OR_LINK_PATTERN = re.compile(r'(?:https?://|#)\S+', re.I)
P_ELEMENT_PATTERN = re.compile(r'<p>(.*?)</p>', re.I)
...
def extract_course_times(course=COURSE):
    """Return the course timings from the passed in
       ...
       Return this list.
    """
    return TIME_PATTERN.findall(course)
...
def get_all_hashtags_and_links(tweet=TWEET):
    """Get all hashtags and links from the tweet text
       ...
       Return this list.
    """
    return HASHTAG_OR_LINK_PATTERN.findall(tweet)
...
def match_first_paragraph(html=HTML):
    """Extract the first paragraph of the passed in
       ...
       Return this string.
    """
    return P_ELEMENT_PATTERN.search(html).group(1)
