...
internal sealed class Graph {
    ...
    internal (IList<int> cycle, int cost)
    FindMaxCostHamiltonianCycle(int start)
    {
        var bestCycle = Enumerable.Repeat(-1, Order).ToArray();
        if (Order < 2) return (bestCycle, 0);
        ...
        void SearchFrom(int src)
        {
            ...
            if (path.Count < Order) {
                for (var dest = 0; dest < Order; ++dest) {
                    ...
                    pathCost = checked(pathCost + _adj[src, dest]);
                    SearchFrom(dest);
                    pathCost = checked(pathCost - _adj[src, dest]);
                }
            } else {
                var cost = checked(pathCost + _adj[src, start]);

                if (cost > bestCost) {
                    path.CopyTo(bestCycle);
                    bestCost = cost;
                }
            }
...
internal sealed class KeyGraph<T> where T : notnull {
    ...
    internal (IList<T> cycle, int cost) FindMaxCostHamiltonianCycleAnySource()
    {
        var (cycle, cost) = _graph.FindMaxCostHamiltonianCycle(0);
        return (cycle.Select(vertex => _keys[vertex]).ToList(), cost);
    }
...
internal static class Program {
    private static void Main(string[] args)
    {
        ...
        var (cycle, cost) = ReadEdges(filespec)
            .ToList() // Fail fast on syntax errors.
            .ToKeyGraph()
            .FindMaxCostHamiltonianCycleAnySource();

        cycle.Dump(nameof(cycle));
