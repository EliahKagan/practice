# Advent of Code 2020, day 12, part B
...
def scale(scalar, vector)
  vector.map { |component| scalar * component }
end

def translate(affine, vector)
  if affine.size != vector.size
    raise ArgumentError.new("dimensions don't agree")
  end
  affine.zip(vector).map { |coordinate, delta| coordinate + delta }
end
...
ARGF.each_line.map(&.strip).reject(&.empty?).each do |line|
  ...
  case command
  when 'F'
    pos = translate(pos, scale(argument, delta))
  when 'R'
    delta = turn_right(delta, argument)
  when 'L'
    delta = turn_left(delta, argument)
  else
    delta = translate(delta, scale(argument, DIRECTIONS[command]))
  end
end
